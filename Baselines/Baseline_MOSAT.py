...
# -*- coding: utf-8 -*-
"""
MOSAT baseline — full runnable entry point:
- GA seeding + end-of-generation SVGD refinement over NPC initial conditions (ds, dd, dyaw).
- MOSAT (motif-driven) online NPC control via atomic maneuver sequences.
- MLP surrogate: input = spawn-point features, label = per-NPC hazard at the closest-approach
  frame inside the episode (min-distance scheme).
- Online fine-tuning each episode for 1-2 epochs; optional EMA if the surrogate provides
  ema_update.
"""
...
# —— Traffic Manager —— #
KEEP_ALIVE_PERIOD = 30  # TM renewal period (ticks).
...
# —— SVGD gating (triggered at end of generation, not per episode) —— #
...
# —— Minimum spawn count (roll back if fewer NPCs are created) —— #
MIN_NPC = 20

# ====================== MOSAT atomic action and motif definitions ======================
ATOMIC_ACTIONS = [
    "accelerate", "break",
...
class AtomicGene:
    """A single atomic maneuver with a duration in seconds."""
...
class MotifGene:
    ...
    Generates a sequence of atomic maneuvers based on the relative configuration
    of the ego and the NPC.
    """
    ...
    def plan(self, world_map, ego: carla.Actor, npc: carla.Actor):
        """Return a sequence of AtomicGene objects for this motif."""
        ...
        front = (dx*cy + dy*sy) > 0  # True if NPC is ahead of the ego.

        # Each motif produces 1–3 atomic maneuvers; nominal duration is 1 s per step.
        ...
        # Default fallback: gentle acceleration or braking.
...
def _pick_motif_for_npc(world_map, ego: carla.Actor, npc: carla.Actor) -> MotifGene:
    """
    Select a motif type based on the relative configuration of ego and NPC.
    """
    ego_wp = world_map.get_waypoint(ego.get_location(), project_to_road=True)
    npc_wp = world_map.get_waypoint(npc.get_location(), project_to_road=True)
    same_lane = (ego_wp.road_id == npc_wp.road_id and ego_wp.lane_id == npc_wp.lane_id)
...
def mosat_plan_sequences(world_map, ego: carla.Actor, vehicles: list):
    """
    Generate an action sequence for each NPC via motif planning.

    Returns:
        dict[vid] = [{"act": str, "steps": int}, ...]
    """
    seqs = {}
    for v in vehicles:
        if v.id == ego.id:
...
def average_population_distance(population, generation):
    """
    Compute the average distance between one individual and all members of the generation.
    """
...
class EpisodeRecorder:
    def __init__(self, world_map):
        self.map = world_map
        self.frames = []  # List of per-tick dicts: [{"ego": {...}, "npcs": {id: {...}}}]
...
def _closing_speed_at(ego_tf, ego_vel, npc_tf, npc_vel):
    ...
    v_close = -(vrelx * ux + vrely * uy)  # Positive value means approaching.
...
def hazard_from_min_distance(frames, world_map, npc_id,
    ...
    # Collision approximation: any frame within the window with distance < 1.5 m → label 1.
    collided = False
    v_closes = []
    heads = []
    for t in idxs:
...
def main():
    ...
    while number_game <= Test_buget:
        abnormal_case = False
        # === Retrieve a seed from the GA population (position_info) ===
        ...
        if not success:
            print("[ERROR] Vehicle spawn failed; skipping episode (not counted as valid).")
        ...
        print(f"[INFO] Requested {scenario_conf.get('vehicle_num','?')} vehicles, spawned {actual_n} (valid episode index {number_game})")
        ...
        # Attach all NPCs to Traffic Manager.
        ...
        # MOSAT runtime state (replaces the original ART block).
        mosat_sequences = None   # dict[vid] -> [{"act": str, "steps": int}, ...]
        mosat_state = {}         # vid -> {"i": current index, "left": remaining steps}
        ...
        for step in range(50000):
            ...
            if step < STARTUP_STEPS:
                ...
                if speed_ego > 5:
                    print('[WARN] Abnormal ego speed during startup phase; ending episode.')
                    abnormal_case = True
            ...
            if step > STARTUP_STEPS:
                # Renew TM control, skipping NPCs currently being overridden.
                ...
                # Destination arrival / overshoot check.
                ...
                # ========== MOSAT: execute atomic action sequences from motif plans ==========
                # Generate the plan on first entry (based on current relative configuration).
                ...
                # Optional: re-plan at a fixed stride for more persistent interference.
                ...
                if mosat_sequences is not None:
                    ...
                    for v in demo.vehicles:
                        ...
                        if not seq or st is None:
                            # No sequence assigned; revert to TM control.
                            v.set_autopilot(True, tm.get_port())
                            continue
                        ...
                        idx = st["i"]
                        if idx >= len(seq):
                            # Sequence exhausted — return vehicle to TM control.
                            v.set_autopilot(True, tm.get_port())
                        ...
                        if st["left"] <= 0:
                            ...
                            if st["i"] < len(seq):
                                ...
                                # All atomic steps complete — return to TM control.
                                v.set_autopilot(True, tm.get_port())
                                finished_vids.append(v.id)

                    # Remove completed vehicle IDs from the state tracker.
                    for vid in finished_vids:
                        mosat_state.pop(vid, None)

                # Tick environment stats; early-exit on collision.
        ...
        # End of episode: flush Apollo prediction/planning cache if present.
        try:
        ...
        if distance_to_start < 1:
            print("[WARNING] Ego start-to-end distance < 1 m — episode flagged as abnormal and excluded from this generation.")
            abnormal_case = True

        # Accumulate statistics and run GA evolution (valid episodes only).
        if not abnormal_case:
            ...
            # Record outcome, including whether a vehicle side-collision occurred.
            ...
            # --- Mid-generation expansion: parent selection + crossover/mutation ---
            if (number_game % population_size) == 0 and (number_game % (2 * population_size) != 0):
                ...
                # Use collision outcome as a coarse safety signal (indicative only; does not affect the GA ranking interface).
                for rec_i in gen_records:
                    fitness["safety_violation"].append(-1 if rec_i["collided"] else 1)
                    fitness["ART_trigger_time"].append(0)  # MOSAT does not track ART triggers; fill with 0.
                print(fitness)
            ...
            # --- Full generation end: SVGD injection + advance to next generation ---
            if (number_game % (2 * population_size)) == 0:
                ...
                for r in gen_records:
                    ...
                    fitness["ART_trigger_time"].append(0)  # MOSAT has no ART component; fill with 0.
            ...
            print(f"[INFO] Abnormal episode detected; scenario resampled and excluded from valid episode count.")
