...
# -*- coding: utf-8 -*-
"""
Baseline: GA + RL (DQN) adversarial NPC testing.
- GA seed sampling + end-of-generation SVGD refinement of NPC initial (ds, dd, dyaw).
- RL (DQN) online NPC control (replaces ART; adaptive policy + combined actions).
- MLP surrogate: input = initial-pose features, label = hazard at minimum-distance
  moment within an episode (min-distance scheme).
- Online per-episode 1-2 epoch fine-tuning; optional EMA (if surrogate provides ema_update).
"""
...
# ====== DQN replaces ART; all other components unchanged ======
from dqn_agent import DQNAgent

# ====================== Global hyperparameters ======================
TIME_STEP = 0.05
population_size = 10  # Valid episodes per generation.

# —— Traffic Manager —— #
KEEP_ALIVE_PERIOD = 30  # Interval (ticks) to renew TM autopilot for non-adversarial vehicles.
...
# —— Minimum spawn count (roll back if fewer NPCs are spawned) —— #
MIN_NPC = 20

# —— Action duration table (consistent with original) —— #
SBART_STEPS_BASE = {
    "break": 10,
    "accelerate": 4,
    "left_change_acc": 10,
...
def average_population_distance(population, generation):
    """Compute the average distance from one population entry to all entries in the generation."""
...
def _closing_speed_at(ego_tf, ego_vel, npc_tf, npc_vel):
    ...
    v_close = -(vrelx * ux + vrely * uy)  # Positive = closing.
...
# ====================== RL (DQN) NPC controller ======================

# Discrete action set (includes "keep" = no override).
RL_ACTIONS = [
    "keep",
    "break",
    "accelerate",
...
class RLNPCController:
    """Use a shared DQN agent to select and train NPC override actions."""
    def __init__(self, agent: DQNAgent, world_map, surrogate: NPCHazardMLPSurrogate,
        ...
        # Trace of last-step overridden NPCs: vid → {"obs": np.ndarray, "a": int}.
        self.traces = {}

    @torch.no_grad()
    def _surrogate_hazard_prob(self, ego_tf, npc_tf):
        # Consistent with training: model output is already a probability (BCELoss, not logits).
    ...
    def select_and_record(self, ego, vehicles):
        """Select NPCs to override and return [(v, act_name)]; also records (s, a) in traces."""
        ego_id = ego.id
        scored = []
        ego_tf = ego.get_transform()
        for v in vehicles:
    ...
    def post_tick_update(self, ego, vehicles, collided: bool):
        """
        Called every tick: for each (s, a) in traces, generate (s', r, done),
        push to replay, and run an optimization step.
...
def main():
    ...
    while number_game <= Test_buget:
        abnormal_case = False
        # === Retrieve the next seed from the GA population ===
        ...
        print(f"[INFO] Requested {scenario_conf.get('vehicle_num','?')}, spawned {actual_n} (valid episode #{number_game})")
        ...
        # Register all NPC vehicles with the traffic manager.
        ...
        # RL runtime state (override mechanism from the original design).
        ...
        for step in range(50000):
            ART_triggered = 0  # Reused as “override occurred” flag.
            world.wait_for_tick()
            ...
            # Startup phase: detect abnormal ego speed.
            ...
            if step > STARTUP_STEPS:
                # Periodically renew TM autopilot (excluding currently overridden NPCs).
                ...
                # Step vehicles currently in override mode.
                dead_ids = []
                ...
                # Step environment and collect status flags.
                ...
                # Non-blocking camera frame save.
                ...
                # Log one replay frame.
                rec.log(demo.ego_vehicle, demo.vehicles)

                # === DQN: use previous (s, a) to compute (s', r, done) and train; then select a new override action ===
                ...
                if (step % 3) == 0:  # Trigger frequency matches the original ART cadence.
                    selected = rl_ctrl.select_and_record(demo.ego_vehicle, demo.vehicles)
                    if selected:
                        ART_triggered = 1  # Reused as “override occurred” flag.
        ...
        if not abnormal_case:
            ...
            # --- Save DQN checkpoint at the end of each episode (optional) ---
            try:
                dqn.save("dqn.pt")
            except Exception:
                pass
