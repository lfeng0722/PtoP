...
- TTC is only computed when the target is directly ahead with a small lateral offset;
  otherwise returns +inf to avoid physically inconsistent values.
- RiskScorer lateral term uses a “centering” score (ly ≈ 0 is most dangerous),
  removing the redundant rvx term.
- approaching_cnt is corrected to count only vehicles directly ahead with rvx < 0.
- Trigger evaluation uses policy's lat_range to recompute TTC, decoupled from
  compute_phi for consistency.
- KNN memory-bank vectors are built with vehicles sorted by id (canonicalize) to
  prevent distance distortion from permutation order.
- All other interfaces remain compatible: ARTSelectorScenario / compute_phi /
  update_adaptive_policy / filter_triggerables / build_actions_for.
“””
...
# ============ Local coordinate and feature helpers ============
...
def _ttc(lx: float, ly: float, rvx: float, lat_range: float = 6.0, eps: float = 1e-3) -> float:
    “””
    Compute longitudinal TTC only when the target is directly ahead (lx > 0) and
    within lateral range (|ly| < lat_range); otherwise returns +inf (approximated
    as 1e6). rvx = ovx - evx in the ego-local frame; closing speed = -rvx.
    “””
...
def compute_phi(ego: "carla.Vehicle", other: "carla.Vehicle") -> np.ndarray:
    """
    Compute the 5-dimensional feature vector φ = [lx_norm, ly_norm, rvx_norm, rvy_norm, inv_ttc] ∈ [0,1]^5.

    Normalization ranges:
      lx ∈ [-40, 40] m, ly ∈ [-10, 10] m, rvx/rvy ∈ [-10, 10] m/s,
      TTC mapped to [0, 1] over a 5-second bandwidth.
    """
...
class RiskScorer:
    ...
    def __call__(self, phi: np.ndarray) -> float:
        “””
        Score a scenario feature vector.

        If a model is provided (outputting logits), uses the model;
        otherwise falls back to a heuristic that weights inv_ttc heavily and
        penalizes lateral offset (ly ≈ 0 is most dangerous).
        “””
        if self.model is not None:
            import torch
            with torch.no_grad():
                x = torch.from_numpy(phi).float().unsqueeze(0)
                y = self.model(x)            # Assumes model output is a logit.
                y = torch.sigmoid(y)
                return float(y.item())

        lx_norm, ly_norm, rvx_norm, rvy_norm, inv_ttc = phi
        # ly_norm ∈ [0,1] → centered at 0.5 means ly ≈ 0 (most dangerous).
...
def _canonicalize(vehicles: List["carla.Vehicle"]) -> List["carla.Vehicle"]:
    """Sort vehicles by id to ensure consistent feature-vector construction."""
...
class ARTSelectorScenario:
    def __init__(
        ...
        # Note: use 'break' (not 'brake') to match the external action_trans naming convention.
        ...
        # Memory banks bucketed by the number of triggered vehicles (1–4).
        self.risk_set: Dict[int, List[Tuple[float, ...]]] = {1: [], 2: [], 3: [], 4: []}
        self.safe_set: Dict[int, List[Tuple[float, ...]]] = {1: [], 2: [], 3: [], 4: []}

    # ---- KNN distance (with feature weights and action scale) ----
    ...
    # ---- Build vector with vehicles sorted by id; actions_list and vehicle_list are 1:1 ----
    def build_global_vector(self, ego_vehicle: "carla.Vehicle",
        ...
        vec: List[float] = []
        # Build id→action mapping, then concatenate in canonical (id-sorted) order.
    ...
    def parse_global_vector_to_actions(self, global_vec: Tuple[float, ...]) -> List[str]:
        """Recover action names from a global vector (in canonical id-sorted order)."""
    ...
    # ---- Memory-bank recording: vehicle_list must match the trigger-set semantics used during selection ----
    ...
    # ---- Core: choose actions for all vehicles in one step (trigger subset selected internally) ----
    def choose_actions_for_all_vehicles(self, ego_vehicle: "carla.Vehicle",
        ...
        # 1) Score vehicles with the heuristic/model and filter to trigger candidates (sorted by score desc).
        ...
        # No triggerable vehicles: assign random actions to all as a fallback.
        if not trigger_list:
        ...
        # 2) KNN memory-bank scoring (search over action combinations for the trigger subset only).
        n = len(trigger_list)
        key = n if n < 4 else 4
        risk_set = self.risk_set[key]
        safe_set = self.safe_set[key]

        # Candidate action sequence: prioritize lateral deceleration/avoidance, then acceleration/deceleration.
        possible_actions = [
        ...
        best_tuple = None  # Cached sort key for the current best combo.

        def sort_key(ds: Optional[float], dr: Optional[float]) -> Tuple[float, float, float]:
            # Prefer large ds (far from safe cases) and small dr (close to risk cases).
        ...
        # 3) Build the full vehicle action list (non-triggered vehicles default to 'accelerate').
...
# ============ Adaptive policy (aggressive baseline) ============
def _safe_count_driving_lanes(wp, max_hops=8, stop_on_road_change=True):
    """Count drivable lanes to the left and right of wp, with a hop limit and visited-set cycle guard."""
    if wp is None:
        return 1
...
def analyze_scene(world_map: "carla.Map", ego: "carla.Vehicle",
    ...
    # At junctions, default to 1 lane to avoid cycling through complex topology.
    at_junction = bool(w and w.is_junction)
    if at_junction or not w:
        lane_cnt = 1
    else:
        # Safe count with hop limit and cycle guard.
    ...
    # Compute forward density and approaching-vehicle count.
    total_cnt = 0
    forward_cnt = 0
    approaching_cnt = 0
    min_ttc = 1e6
    ...
    def _ttc(lx, ly, rvx, lat_range=6.0, eps=1e-3):
        # Use longitudinal TTC only when directly ahead with small lateral offset.
        if not (lx > 0 and abs(ly) < lat_range):
            return 1e6
        rel_speed_long = -rvx
        if rel_speed_long <= eps:
...
def adapt_triggers(stats: Dict[str, float]) -> Dict[str, float]:
    ...
    # Emergency mode: trigger earlier when TTC is critically low.
    if min_ttc < 2.0:
...
# ============ Trigger filtering and entry points ============
...
def filter_triggerables(ego: "carla.Vehicle", vehicles: List["carla.Vehicle"],
    ...
    """
    Filter vehicles to trigger targets using the ROI and thresholds from policy.
    Returns the filtered list sorted by risk score descending.

    Note: choose_actions_for_all_vehicles also performs internal trigger filtering;
    pass the result of this function as vehicle_list only when you need explicit
    control over the trigger subset.
    """
    ego_tf = ego.get_transform()
...
def build_actions_for(art: ARTSelectorScenario, ego: "carla.Vehicle",
    ...
    """
    Choose actions via the ART selector.

    If full_vehicle_list is provided, actions are chosen for all vehicles (with the
    trigger subset selected internally); otherwise actions are chosen only for
    trigger_vehicles.
    """
